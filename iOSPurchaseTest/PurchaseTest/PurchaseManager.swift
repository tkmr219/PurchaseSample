...
//  PurchaseTest
//
//  Created by 竹村弘己 on 2018/03/29.
//  Copyright © 2018年 Trifort. All rights reserved.
//
...
struct PurchaseManagerErrors: OptionSet, Error {
    ...
    static let cannnotMakePayments  = PurchaseManagerErrors(rawValue: 1 << 0)
    ...
    public var localizedDescription: String {
        ...
        if self.contains(.cannnotMakePayments) {
...
open class PurchaseManager : NSObject {
    ...
    public func purchase(product: SKProduct) {
        ...
        if SKPaymentQueue.canMakePayments() == false {
            errors.insert(.cannnotMakePayments)
        }
    ...
    public func restoreTransaction(_ transaction : SKPaymentTransaction) {
        // リストア（originalTransaction を didFinishPurchaseWithTransaction で通知 ※設計に応じて変更
        delegate?.purchaseManager(self, didFinishTransaction: transaction, decisionHandler: {(complete) -> Void in
            if complete == true {
                // トランザクション終了
                SKPaymentQueue.default().finishTransaction(transaction)
    ...
    public func deferredTransaction(_ transaction : SKPaymentTransaction) {
        // 承認待ち
